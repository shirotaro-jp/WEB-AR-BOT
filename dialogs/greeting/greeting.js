...
class Greeting extends ComponentDialog {
    ...
    async promptForNameStep(step) {
        ...
        if (!userProfile.name) {
            ...
            return await step.prompt(NAME_PROMPT, '初めまして。名前は？');
        } else {
            return await step.next();
        }
    }
    ...
    async promptForCityStep(step) {
        ...
        if (!userProfile.city) {
            return await step.prompt(CITY_PROMPT, `やあ、 ${ userProfile.name }。 どこ住んどるん？`);
        } else {
            return await step.next();
        }
    }
    ...
    async greetUser(step) {
        ...
        await step.context.sendActivity(`やあ、 ${ userProfile.name }。 ${ userProfile.city }出身なんだね。よろしく！`);
